...
const Map = ({
  ...
  return <div className="relative w-full h-[500px] rounded-lg overflow-hidden shadow-md">
      {isMapTokenInputVisible ? <div className="absolute inset-0 flex items-center justify-center bg-gray-100 p-4">
          <form onSubmit={handleSubmitToken} className="w-full max-w-md space-y-4">
            ...
            <div className="flex flex-col space-y-2">
              ...
              <input id="mapToken" name="mapToken" type="text" placeholder="Enter your map API token" className="px-3 py-2 border rounded-md" />
            </div>
            <Button type="submit" className="w-full text-xl">Initialize boost to mars
        </Button>
          </form>
        </div> : <>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-full h-full p-4 flex flex-wrap justify-around items-center">
              {sellers.map(seller => <div key={seller.id} className="cursor-pointer transform hover:scale-110 transition-transform" onClick={() => onSellerSelect(seller)}>
